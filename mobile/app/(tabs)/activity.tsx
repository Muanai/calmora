...
export default function ActivityScreen() {
  ...
  const loadGrounding = useCallback(async () => {
    if (!user?.id) return;
    await fetchGrounding(user.id, getToken);
  }, [user?.id]);
          ...
          {isLoading ? (
            ...
            <View className="items-center justify-center pt-8">
              ...
              <Text className="font-jakarta-bold text-[20px] text-black text-center mt-6 mb-3">
                Kamu Hebat Banget! Bagaimana Perasaanmu?
              </Text>
              <Text className="font-jakarta-regular text-[14px] text-[#A7A7A7] text-center leading-[21px] px-2 mb-8">
                Apakah pikiran dan tubuhmu sudah kembali pulih? Jika belum, jangan ragu untuk mencoba kembali yaaa!
              </Text>
              ...
              <TouchableOpacity
                ...
                className="w-[200px] h-[48px] bg-[#D7385E] rounded-[16px] items-center justify-center mt-2"
              >
